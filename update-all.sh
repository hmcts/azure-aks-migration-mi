...
rg --no-line-number --no-heading  --no-filename "releaseName: (.*)" -r '$1' cnp-flux-config/k8s/prod/common cnp-flux-config/k8s/prod/cluster-00 cnp-flux-config/k8s/prod/cluster-01 | awk '{$1=$1};1' | sort | uniq > aks-apps.txt
rm -rf cnp-flux-config/
